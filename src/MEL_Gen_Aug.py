...
class MEL_Gen(keras.utils.Sequence):
    ...
    def _get_one(self, ix):
        ...
        if self.convert_to_rgb:
            if self.img_dtype == "uint8":
                ...
                mel_spec = np.repeat(
                    np.expand_dims(mel_spec.astype(np.float32), 2), 3, 2
                )
    ...
    def noise(self, mel):
        ...
        if np.random.random() < 0.2:
            mel = mel + (
                np.random.sample((self.mel_image_size, self.mel_image_size)).astype(
                    np.float32
                )
                + 8
            ) * mel.mean() * level_noise * (np.random.sample() + 0.2)
        if np.random.random() < 0.2:
            r = np.random.randint(self.mel_image_size // 2, self.mel_image_size)
            x = np.random.random() / 2
            pink_noise = np.array(
                [
                    np.concatenate(
                        (1 - np.arange(r) * x / r, np.zeros(self.mel_image_size - r) - x + 1)
                    )
                ]
            ).T
            mel = mel * pink_noise
        if np.random.random() < 0.2:
            ...
            mel[:, a:b] = mel[:, a:b] + (
                np.random.sample((self.mel_image_size, b - a)).astype(np.float32) + 9
    ...
    def mix_class(self, mel, class_idx):
        ...
        try:
            ...
            if mel_mix.shape != (self.mel_image_size, self.mel_image_size):
